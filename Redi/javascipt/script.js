...
array.forEach((value,index,array) => array[index]=value.toLowerCase());
console.log(array);

let secondArray = array.filter(nome=>nome.startsWith('g'));
console.log(secondArray);

let saluti = array.map(nome=>`Ciao ${nome}`); //Mappaun nuovo array
console.log(saluti);

mynum = [2,4,6,8,10];
found = mynum.find(element=>element>5); //Primo elemento maggiore di 5
console.log(found);

num = mynum.reduce((accumulatore,numero)=>{
    return accumulatore + numero;
},0); //Somma tutti i numeri dell'array; Il secondo parametro di reduce è il valore iniziale dell'accumulatore
